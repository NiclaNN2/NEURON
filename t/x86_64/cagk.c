...
extern void _cvode_abstol( Symbol**, double*, int);
 ...
 _mechtype = nrn_get_mechtype(_mechanism[1]);
  ...
  hoc_register_prop_size(_mechtype, 9, 5);
 	...
 	ivoc_help("help ?1 cagk /Users/Nicolas/Desktop/modeles_NEURON/ca3b_T/x86_64/cagk.mod\n");
...
double exp1 (  double _lk , double _ld , double _lv ) {
   double _lexp1;
 _lexp1 = _lk * exp ( - 2.0 * _ld * FARADAY * _lv / R / ( 273.15 + temperature ) ) ;
 ...
 v=_v;
{
 ...
 for (; t < _break; t += dt) {
 error =  state();
 if(error){fprintf(stderr,"at line 61 in file cagk.mod:\n	SOLVE state METHOD cnexp\n"); nrn_complain(_p); abort_run(error);}
