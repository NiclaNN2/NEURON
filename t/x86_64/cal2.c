...
extern void _cvode_abstol( Symbol**, double*, int);
 ...
 _mechtype = nrn_get_mechtype(_mechanism[1]);
  ...
  hoc_register_prop_size(_mechtype, 10, 5);
 	...
 	ivoc_help("help ?1 cal /Users/Nicolas/Desktop/modeles_NEURON/ca3b_T/x86_64/cal2.mod\n");
...
double ghk (  double _lv , double _lci , double _lco ) {
   double _lghk;
 double _lnu , _lf ;
 _lf = KTF ( _threadargscomma_ temperature ) / 2.0 ;
   _lnu = _lv / _lf ;
...
double KTF (  double _ltemperature ) {
   double _lKTF;
 _lKTF = ( ( 25. / 293.15 ) * ( _ltemperature + 273.15 ) ) ;
...
static int  rate (  double _lv ) {
   double _la , _lb , _lqt ;
 _lqt = pow( q10 , ( ( temperature - 25.0 ) / 10.0 ) ) ;
 ...
 v=_v;
{
 ...
 for (; t < _break; t += dt) {
 error =  state();
 if(error){fprintf(stderr,"at line 58 in file cal2.mod:\n	SOLVE state METHOD cnexp\n"); nrn_complain(_p); abort_run(error);}
