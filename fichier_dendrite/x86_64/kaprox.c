...
extern void _cvode_abstol( Symbol**, double*, int);
 ...
 _mechtype = nrn_get_mechtype(_mechanism[1]);
  ...
  hoc_register_prop_size(_mechtype, 11, 4);
 	...
 	ivoc_help("help ?1 kap /Users/Nicolas/Desktop/NEURON/ca3b_celsius_t/x86_64/kaprox.mod\n");
...
double alpn (  double _lv ) {
 ...
 _lzeta = zetan + pw / ( 1.0 + exp ( ( _lv - tq - sh ) / qq ) ) ;
   _lalpn = exp ( 1.e-3 * _lzeta * ( _lv - vhalfn - sh ) * 9.648e4 / ( 8.315 * ( 273.16 + temperature ) ) ) ;
...
double betn (  double _lv ) {
 ...
 double _lzeta ;
 _lzeta = zetan + pw / ( 1.0 + exp ( ( _lv - tq - sh ) / qq ) ) ;
   _lbetn = exp ( 1.e-3 * _lzeta * gmn * ( _lv - vhalfn - sh ) * 9.648e4 / ( 8.315 * ( 273.16 + temperature ) ) ) ;
...
double alpl (  double _lv ) {
   double _lalpl;
 _lalpl = exp ( 1.e-3 * zetal * ( _lv - vhalfl - sh ) * 9.648e4 / ( 8.315 * ( 273.16 + temperature ) ) ) ;
...
double betl (  double _lv ) {
   double _lbetl;
 _lbetl = exp ( 1.e-3 * zetal * gml * ( _lv - vhalfl - sh ) * 9.648e4 / ( 8.315 * ( 273.16 + temperature ) ) ) ;
...
static int  rates (  double _lv ) {
   double _la , _lqt ;
 _lqt = pow( q10 , ( ( temperature - 24.0 ) / 10.0 ) ) ;
