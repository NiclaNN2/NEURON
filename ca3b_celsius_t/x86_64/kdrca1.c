...
extern void _cvode_abstol( Symbol**, double*, int);
 ...
 _mechtype = nrn_get_mechtype(_mechanism[1]);
  ...
  hoc_register_prop_size(_mechtype, 9, 4);
 	...
 	ivoc_help("help ?1 kdr /Users/Nicolas/Desktop/NEURON/ca3b_celsius_t/x86_64/kdrca1.mod\n");
...
double alpn (  double _lv ) {
   double _lalpn;
 _lalpn = exp ( 1.e-3 * zetan * ( _lv - vhalfn - sh ) * 9.648e4 / ( 8.315 * ( 273.16 + temperature ) ) ) ;
...
double betn (  double _lv ) {
   double _lbetn;
 _lbetn = exp ( 1.e-3 * zetan * gmn * ( _lv - vhalfn - sh ) * 9.648e4 / ( 8.315 * ( 273.16 + temperature ) ) ) ;
 ...
 v=_v;
{
 ...
 for (; t < _break; t += dt) {
 error =  states();
 if(error){fprintf(stderr,"at line 47 in file kdrca1.mod:\n	SOLVE states METHOD cnexp\n"); nrn_complain(_p); abort_run(error);}
