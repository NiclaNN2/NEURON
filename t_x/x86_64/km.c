...
extern void _cvode_abstol( Symbol**, double*, int);
 ...
 _mechtype = nrn_get_mechtype(_mechanism[1]);
  ...
  hoc_register_prop_size(_mechtype, 10, 4);
 	...
 	ivoc_help("help ?1 km /Users/Nicolas/Desktop/NEURON/ca3b_celsius_t/x86_64/km.mod\n");
...
static int  rate (  double _lv ) {
   double _la , _lqt ;
 _lqt = pow( q10 , ( ( temperature - 35.0 ) / 10.0 ) ) ;
 ...
 v=_v;
{
 ...
 for (; t < _break; t += dt) {
 error =  state();
 if(error){fprintf(stderr,"at line 56 in file km.mod:\n	SOLVE state METHOD cnexp\n"); nrn_complain(_p); abort_run(error);}
