...
extern void _cvode_abstol( Symbol**, double*, int);
 ...
 _mechtype = nrn_get_mechtype(_mechanism[1]);
  ...
  hoc_register_prop_size(_mechtype, 14, 4);
 	...
 	ivoc_help("help ?1 na3 /Users/Nicolas/Desktop/NEURON/ca3b_celsius_t/x86_64/na3n.mod\n");
...
double alps (  double _lv ) {
   double _lalps;
 _lalps = exp ( 1.e-3 * zetas * ( _lv - vhalfs - sh ) * 9.648e4 / ( 8.315 * ( 273.16 + temperature ) ) ) ;
...
double bets (  double _lv ) {
   double _lbets;
 _lbets = exp ( 1.e-3 * zetas * gms * ( _lv - vhalfs - sh ) * 9.648e4 / ( 8.315 * ( 273.16 + temperature ) ) ) ;
 ...
 v=_v;
{
 ...
 for (; t < _break; t += dt) {
 error =  states();
 if(error){fprintf(stderr,"at line 72 in file na3n.mod:\n        SOLVE states METHOD cnexp\n"); nrn_complain(_p); abort_run(error);}
